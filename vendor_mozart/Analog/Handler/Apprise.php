 ...
 *     Analog::handler (SedoxWPCatalogue\Dependencies\Analog\Handler\Apprise::init ($command, $service));
 *     
 *     Analog::log ('Log me');
 *
 * Notes:
	...
	public static function init ($command, $service) {
		return function ($info) use ($command, $service) {
			...
			exec (
				sprintf ('%s -b %s %s',
					$command,
					escapeshellarg (vsprintf (\SedoxWPCatalogue\Dependencies\Analog\Analog::$format, $info)),
					$service
				)
			);
		};
