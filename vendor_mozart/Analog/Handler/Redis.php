 ...
 *     $key = 'logs';
 *     Analog::handler (SedoxWPCatalogue\Dependencies\Analog\Handler\Redis::init ($redis, $key));
 */
class Redis {
	public static function init ($redis, $key) {
		return function ($info) use ($redis, $key) {
			$redis->rpush ($key, trim (vsprintf (\SedoxWPCatalogue\Dependencies\Analog\Analog::$format, $info)));
		};
	}
}